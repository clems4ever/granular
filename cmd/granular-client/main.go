// Command granular is the granular CLI client. It is one implementation of the client
// SDK (package client): it reads a YAML configuration listing the authorization server
// and the known gateways, then exposes commands to catalog the gateways' permission
// schemas, run operations (executed when authorized, a clear error otherwise), sign a
// grant request against a gateway, and pack one or more signed grant requests into a
// proposal submitted to the AS for human approval.
//
...
func newRootCmd(out io.Writer) *cobra.Command {
	...
	root.AddCommand(a.catalogCmd(), a.opCmd(), a.signCmd(), a.proposeCmd())
	return root
}
...
func (a *app) catalogCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Print the permission schema of the gateways",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runCatalog(context.Background(), a.c, args, a.out)
		},
	}
}
...
func (a *app) signCmd() *cobra.Command {
	var gatewayID, reason, resType, match, out string
	var actions []string
	cmd := &cobra.Command{
		Use:   "sign",
		Short: "Build a grant request and have a gateway sign it",
		RunE: func(cmd *cobra.Command, args []string) error {
			req := buildGrantRequest(reason, actions, resType, parseMatch(match))
