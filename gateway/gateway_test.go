...
// newGateway builds a gateway over testSchema with the fake operation and a stub verifier.
//
// @arg allow Whether the stub verifier allows operations.
// @return *Gateway The gateway under test.
// @return *stubVerifier The injected verifier.
//
// @testcase TestOperationAllowed builds a gateway with this helper.
func newGateway(allow bool) (*Gateway, *stubVerifier) {
	reg := NewRegistry()
	reg.Register("t.read", func(map[string]any) (Operation, error) { return fakeOp{}, nil })
	sv := &stubVerifier{allow: allow}
	g := New(Config{Schema: testSchema(), Registry: reg, GatewayID: "gw", Secret: []byte(gwSecret), Verifier: sv})
	return g, sv
}
...
// under the gateway secret and is attributed to the gateway.
func TestSignProducesVerifiableRequest(t *testing.T) {
	...
	if sgr.GatewayID != "gw" || !sgr.Verify([]byte(gwSecret)) || len(sgr.Policies) == 0 {
