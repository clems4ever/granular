// Package resource server is the SDK for building a granular resource server (Resource Server). A
// concrete resource server is defined by three domain-specific things the SDK user supplies —
...
// Template is a resource server-authored, parameterized permission shape a client can instantiate
...
// author (Fixed). The resource server expands a template plus its bindings into a single permit
...
// Schema is the permission vocabulary a resource server exposes. Resources, Groups and Actions
