...
// scope with the schema's ScopeFunc. The resource server signs these so a client cannot tamper
// with them.
//
...
// the resource server sends to the AS: one verify.Request per requirement, naming the agent
