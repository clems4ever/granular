...
// TestResourceTreeOrder checks ResourceTree places each resource exactly one depth below its parent.
...
// TestHasActionAndResourceEntity checks HasAction recognises concrete actions and groups and that resource entities resolve.
...
// TestActionLatticeCoversGroupsAndActions checks ActionLattice wires every action and group to its parent groups.
...
// TestVerbGroupsExpand checks VerbGroups expands each group to the concrete actions it covers.
