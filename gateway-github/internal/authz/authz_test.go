...
// TestRwOnPullRequests checks a grant allowing read on a repo and read/write on its
// pull requests.
...
// TestPushOnlyToFeatureBranches checks a branch-name matcher restricting pushes to
// feature/* branches.
...
// TestReadOnlyOpenBugIssues checks attribute matchers limiting reads to open issues
// labelled bug.
...
// TestOrgWideReadViaHierarchy checks an org-scoped grant covers issue reads across all
// its repos through the entity hierarchy.
...
// TestCommentsRequireSeparateCapability checks reading an issue's comments needs a
// capability distinct from viewing the issue itself.
...
// TestIncrementalGrantAddsWrite checks appending a write capability to an existing
// grant without replacing what it already allows.
