// Package gatewaygithub is the concrete GitHub gateway built on the generic gateway
...
func Schema() gateway.Schema {
	...
	for _, r := range cat.Resources {
		match := make([]gateway.MatchField, len(r.Match))
		for i, m := range r.Match {
			match[i] = gateway.MatchField{Name: m.Name, Type: m.Type, Description: m.Description}
		}
		s.Resources = append(s.Resources, gateway.ResourceType{
			Name: r.Name, Title: r.Title, Entity: r.Entity, Parent: r.Parent,
			Description: r.Description, Match: match,
		})
	}
	for _, g := range cat.Groups {
		s.Groups = append(s.Groups, gateway.Group{Name: g.Name, Title: g.Title, Description: g.Description, Parents: g.Parents})
	}
...
func scope(sel gateway.ResourceSelector) (string, string, string, error) {
...
// adapt turns a GitHub operations.Factory into an SDK gateway.Factory, binding the
...
// @return gateway.Factory An SDK factory building an adapted operation.
//
// @testcase TestRegistryBuildsCloneOperation exercises an adapted factory.
func adapt(factory operations.Factory, env operations.Env) gateway.Factory {
	return func(params map[string]any) (gateway.Operation, error) {
...
// adapted wraps a GitHub operations.Operation as a generic gateway.Operation, exposing
...
//
// @return []gateway.Requirement The converted requirements.
//
// @testcase TestRegistryBuildsCloneOperation checks the converted clone requirement.
func (a adapted) Requirements() []gateway.Requirement {
	in := a.op.Requirements()
	out := make([]gateway.Requirement, len(in))
	for i, r := range in {
		out[i] = gateway.Requirement{Action: r.Action, Resource: convertRef(r.Resource), Context: r.Context}
	}
	return out
}
...
// @return gateway.ResourceRef The equivalent SDK resource reference.
//
// @testcase TestRegistryBuildsCloneOperation converts a repo resource chain.
func convertRef(r authz.ResourceRef) gateway.ResourceRef {
	out := gateway.ResourceRef{Type: r.Type, ID: r.ID, Attrs: r.Attrs}
	if r.Parent != nil {
