...
func Build() Catalog {
	return Catalog{
		...
		Groups: []Group{
			...
			{Name: "pulls.read", Title: "pulls:read", Parents: []string{"read"}, Description: "List and view pull requests."},
			{Name: "pulls.write", Title: "pulls:write", Parents: []string{"write"}, Description: "Create pull requests."},
		},
		Actions: []Action{
			...
			{"repo.push", "Push to branch", "github.branch", []string{"write"}, "", true, "per repository + branch matcher", "Push commits to a branch (planned)."},
			...
			{"pull.list", "List pull requests", "github.repo", []string{"pulls.read"}, "", false, "per repository + state", "List a repository's pull requests (planned)."},
